...
* [`src/cljs/hs_client/`](src/cljs/hs_client/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/hs_client/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/hs_client/`](test/cljs/hs_client/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `hs-client` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/hs_client/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns hs-client.example
  (:require [hs-client.config :as config])
